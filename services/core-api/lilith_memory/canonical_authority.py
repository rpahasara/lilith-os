...
class RollbackAuthority:
    ...
    def resolve_and_consume_in_transaction(
        self,
        ...
        *,
        ...
        """Resolve and consume rollback authority inside the caller's transaction.

        The connection must already be in an explicit transaction.  This seam
        lets L04 make the one-time consumption atomic with the new revision;
        this method never commits or rolls back the caller's transaction.
        """
        if not conn.in_transaction:
            raise RollbackError("rollback consumption requires an active transaction")
        row = conn.execute(
